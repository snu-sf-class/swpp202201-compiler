...
tryParseValue(const std::string_view __value_str) noexcept {
  ...
  if (prefix == 'r' || prefix == 's') {
    ...
    auto res_mapped = decltype(res)::map<ValueTy>(
        std::move(res), [](auto &&reg) { return ValueTy(std::move(reg)); });
    return decltype(res_mapped)::mapErr<ParseValueError>(
        std::move(res_mapped),
...
tryCalculateAsyncLoadAccessWidth(llvm::CallInst &__inst) noexcept {
  ...
  if (ret_ty->isIntegerTy() && __inst.arg_size() == 1) {
    ...
    if (arg_ty && !arg_ty->isOpaque() &&
      ...
      return decltype(aw_res)::mapErr<IllFormedIntrinsicError>(
          std::move(aw_res),
